    ...
    <div class="w-full px-4 sm:px-6 lg:px-8">
        <div class="flex h-20 items-center justify-between lg:grid lg:h-24 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
            ...
            <div class="flex items-center gap-2 sm:gap-3 lg:justify-self-end">
                <form action="{{ request()->getBaseUrl() }}/products" method="GET" class="relative md:hidden" data-unified-search data-search-url="{{ request()->getBaseUrl() }}/api/search" data-products-url="{{ request()->getBaseUrl() }}/products" data-categories-url="{{ request()->getBaseUrl() }}/categories">
                    <div class="relative">
                    ...
                    <div data-search-results role="listbox" class="absolute right-0 top-full mt-2 hidden max-h-[70vh] w-[calc(100vw-2rem)] max-w-sm overflow-y-auto rounded-2xl border border-gray-100 bg-white p-2 shadow-2xl dark:border-gray-700 dark:bg-gray-800"></div>
                </form>
